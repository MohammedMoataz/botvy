...
| **`sqlite3` and `drift_flutter` upgraded** by `flutter pub upgrade --major-versions` | Dependencies did not resolve at all — `sqlite3` was pinned below drift's requirement | Pin the set by hand in `apps/mobile/pubspec.yaml` |
| **API-client codegen switched off** in `apps/mobile/build.yaml` | It reads `packages/contracts/openapi.json` and aborts the whole codegen run on a fresh clone, taking drift's generation down with it | Enable it in the same change that starts committing generated contracts |
