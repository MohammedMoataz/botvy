...
if ($Json) {
    ...
    # LOCAL EDIT (botvy, E-002). Same edit, same reason, as check-prerequisites.ps1: spec-kit
    # generates these piped to `| Out-Null`, which discards the helper's own `[OK] research.md`
    # lines and prints the AVAILABLE_DOCS: header over nothing. The pipe exists because
    # Test-FileExists writes the line *and* returns a boolean into the same stream, so the
    # boolean is filtered by type instead. `--json` is untouched. Generated by spec-kit and
    # hash-tracked in .specify/integrations/claude.manifest.json — a future `specify` run will
    # overwrite this; reapply it rather than wondering.
    $(
