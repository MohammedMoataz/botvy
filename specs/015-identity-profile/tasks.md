...
- [X] T117 [P] The `me` query (role, status, `deviceCount`) and admin `users` (search, status and role filters, cursor) — written, not extended: P0 shipped no `me` query to extend. `MeQueryHandler` is a slice of its own because `deviceCount` comes from the devices query, and a resolver reaching into two repositories itself would be a second place deciding what a member looks like. Also `myDevices`, `devicesOf(userId)` (admin), `profile`, `bodyMetrics(last:)`, `preferences` and admin `settings` — nine queries, each a thin adapter over the query handler its REST read already uses.
