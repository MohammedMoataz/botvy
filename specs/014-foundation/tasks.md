...
- [X] T025 `graphql/`: `graphql.module.ts` (code-first, Apollo, in-memory schema, scalars, error code mapping) — **built in the pre-016 review pass, not in P0.** P0 shipped `graphql/scalars.ts` and nothing else, so there was no schema, no resolver and no `me`. The DataLoader factory is deliberately not there: P1's reads have no N+1 to solve, and a loader for one is a dependency bought on speculation. `GqlAuthGuard` is not a separate class either — `JwtAuthGuard`, `KindGuard` and `RolesGuard` were already written to read a GraphQL context through `principalFrom`, so the global guards cover this transport. It needed `@nestjs/graphql@14` and `@nestjs/apollo@14`: the pinned 13.x line peers on Nest 11 and deep-imports `@nestjs/core/injector/internal-core-module`, which Nest 12's exports map rewrites to a file that does not exist — a runtime failure, not a warning.
- [X] T026 [P] `ws/`: `socket.gateway.ts` (`/ws`, `WsAuthGuard` in Socket.IO middleware rather than `handleConnection`, room `user:<id>` for everyone and `ops` for admins, `presence.ping`, `sync.subscribe`, `auth.expiring` then `token_expired`, refuse service principals), `nudge.service.ts`, `ws.module.ts` — **built in the pre-016 review pass.** P0 shipped `nudge.service.ts` and `ws-auth.guard.ts` and provided neither in any module, so `NudgeService.attached` was false in both roles, every nudge in the application went nowhere, and all three clients connected to a path that answered nothing. The guard runs in middleware and not in `handleConnection` because only a middleware rejection reaches the client as a `connect_error` carrying a code — a disconnect from inside the handler is indistinguishable from the network dropping, and a client that cannot tell those apart reconnects forever with the same dead token. 14 specs.
...
- [X] T031 `gen:contracts`: generation mode in `main.ts` writes `packages/contracts/openapi.json`, `schema.graphql`, `events/*.schema.json` (zod → JSON schema for `Pinged`, `SettingChanged`); `packages/contracts/scripts/generate.mjs` emits TS (F-12, FR-014) — `schema.graphql` first written in the pre-016 review pass, once resolvers existed. It is built with `GraphQLSchemaFactory` from the resolver classes rather than by the driver's `autoSchemaFile`: the driver writes its file as a side effect of *serving*, and this mode never listens, so a path in the module produced no file and no error either. `openapi.json` had also gone stale by 26 of 30 operations and is regenerated.
...
- [X] T040 [US1] `infra/docker-compose.yml`: services `caddy`, `frontend`, `backend`, `worker`, `postgres`, `mongo` (`--replSet rs0`, healthcheck `infra/mongo/init-replica.sh`, which initiates the set on first boot and is a status read afterwards), `backups` (built from `infra/backup/Dockerfile`, because `mongo:8` ships neither `cron` nor `pg_dump`; cron `${BACKUP_CRON:-0 3 * * *}` runs both dump scripts → `./backups`, pruning past `BACKUP_RETENTION_DAYS`), `n8n` (Postgres-backed, `ports: ${N8N_BIND:-127.0.0.1:5679}:5678` — a loopback publish, which principle V permits, settable or removable where v1's n8n already holds the port; environment `BOTVY_INTERNAL_TOKEN=${INTERNAL_SERVICE_TOKEN}` and `BOTVY_WEBHOOK_SECRET=${AUTOMATION_WEBHOOK_SECRET}` so the committed workflows need no stored credential), `cloudflared` (profile `tunnel`); every required var `${VAR:?}`; `caddy` is the only service publishing beyond loopback; volumes `pg_data`, `mongo_data`, `n8n_data`, `media`, `caddy_data` — spec: `init-replica.sh` run twice initiates once (F-03, F-05, FR-001/004/005)
- [X] T041 [P] [US1] `infra/Caddyfile` (`{$CADDY_SITE}`; `/api/* /graphql /ws /health /media` → `backend:8080` with WebSocket upgrade; else → `frontend:3000`), `infra/.env.example` documenting every variable in data-model §7, `infra/postgres/001_create_databases.sh` (ported; a shell script rather than plain SQL so it can read `N8N_DB`), `infra/backup-postgres.sh` (nightly `pg_dump` beside the Mongo dump, same retention window — FR-005)
...
- [X] T051 [P] [US2] `apps/frontend`: `app/(marketing)/page.tsx`, `app/(admin)/layout.tsx` + `login/page.tsx` (form → `AuthStore.login`; shows "sign-in arrives in the next phase" on 404), `stores/provider.tsx` (per-request store, `enableStaticRendering`), PrimeReact theme + `tokens.css`, `next.config.ts`; en/ar via `next-intl` with `dir` switch
...
- [X] T053 [P] [US2] `apps/mobile`: `lib/main.dart`, `lib/app/{router.dart (go_router: /sign-in), theme.dart (FlexColorScheme from BotvyTokens), di.dart (get_it), l10n/ (en, ar)}`, `lib/core/db/database.dart` (`SyncColumns` mixin, `KeyValues`, `schemaVersion 1`, `MigrationStrategy` skeleton), `lib/core/api/api_client.dart` (dio, ported from `legacy/apps/mobile/lib/src/api/api_client.dart`) + `socket_client.dart` (**new** — v1 streams over `sse.dart`, so there is no socket client to port; written against `socket_io_client` and the blueprint's `ws-chat.md` handshake), base URL from `--dart-define` then secure storage (**and a screen that can set it, added in the pre-016 review pass** — P0 and P1 built `readBaseUrl`, `writeBaseUrl`, the `origin` setter and the build define, and no way at all for a person to change the value, so every real handset ran against the Android emulator's loopback to its own host and a self-hosted platform could not be pointed at the host hosting it without a new APK; `features/settings/presentation/server_page.dart`, reachable from the sign-in screen because that is where a wrong address shows up),  `lib/core/notifications/` + `lib/core/push.dart` (ports from legacy), `lib/features/auth/presentation/sign_in_page.dart`; tests: `test/migration_ladder_test.dart` (the two assertions in data-model §4), `test/base_url_test.dart` (ported) (FR-013/015)
- [X] T054 [US2] Prove the root scripts T002 declared: `pnpm dev:*`, `build`, `test`, `lint` and `gen:contracts` each run from a clean checkout; `pnpm gen:contracts` produces files and `pnpm -r build` compiles frontend + extension against them; `swagger_dart_code_generator` configured in mobile `build.yaml` (FR-014)
