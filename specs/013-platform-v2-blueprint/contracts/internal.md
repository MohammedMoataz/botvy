...
| `POST /internal/knowledge/ingest` | ingest | **Added in P7.** One pass: return what a dead worker left mid-pipeline, then read what is waiting. Two requirements written after this contract need a *periodic* pass — FR-016, because a link left mid-read must come back on its own, and FR-017, because a job that stops running must be visible and a heartbeat only exists if something stamps it. There is nothing for the per-link route to stamp | `{ requeued, ingested, failed, ms }` |
| `POST /internal/knowledge/ingest/:linkId` | ingest | Re-run the ingestion pipeline for one link regardless of state. Spends no attempt: the Owner asking again is not the source refusing | `{ status }` |
