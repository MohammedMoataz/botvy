...
| `training.SportsChanged` / `SlotsChanged` | Training | `{ sports }` / `{ slots }` | Training (self) → re-materialise sessions over `training.materialiseDays` (a registry key **P6 adds** — the blueprint said P0 registered it and it was not there) |
...
| `training.SessionCompleted` | Training | `{ sessionId, at, sport }` | Notifications → drop alerts; Rhythm → today's plan snapshot; Knowledge → mark suggestion outcome |
| `training.SessionCancelled` / `SessionSkipped` / `SessionDeleted` | Training | `{ sessionId, at }` | Notifications → drop alerts. A session the materialiser removes with its vanished slot raises `SessionDeleted`, so the reconcile needs no branch of its own |
| `training.SessionRescheduled` | Training | the same payload as `SessionScheduled` | Notifications → move the alert. Raised by an edit to the moment, the length, the sport or the title, and by the **time-zone recompute**: the instant that was 18:00 in Cairo is not 18:00 in Berlin, and nothing re-reads the slot at alert time |
| `training.SessionLogged` | Training | `{ sessionId, exerciseId, sets }` | (informational; nothing subscribes. Logging is not completing — a member part way through has logged sets and a `planned` status) |
| `training.ProgramCreated` / `ProgramArchived` / `ProgramDeleted` | Training | `{ programId, … }` | (informational; archiving stops the materialiser *consulting* the program and rewrites nothing it already filled — FR-008) |
| `training.ProgramApplied` | Training | `{ programId, title, startDate, weeks }` | Training (self) → fill sessions **as the horizon reaches each week**, not all at apply time, which is what lets a program outlive `training.materialiseDays` (FR-008); Conversations → coach message |
...
| `knowledge.LinkIngested` | Knowledge | `{ linkId, docId, tags }` — **`docId` is nullable**: an expanded playlist’s *parent* finishes with children rather than a document, and a payload that promised one would make every consumer branch on a value it had been told was there | Knowledge (self) → children/playlist rollup. **No coach message**, corrected in P7: the rhythm writes its touches into the chat because the *question* would otherwise exist only inside a notification, and a link has no question — a line about every link a member saves is Botvy narrating its own background work |
| `knowledge.SuggestionReady` | Knowledge | `{ suggestionId, forDate, sport, sessionId }` | Notifications → alert `suggestion`, timed for *now* rather than for the session: the point of telling the member is that they still have time to act. **No coach message**, corrected in P7 for the reason `LinkIngested` gives — a suggestion has a screen, the inbox, with Use this and No thanks on the card. The one thing that *does* write into the chat is the opposite case: a draft the model would not produce structurally, where there is no card to show and its plain words are the only honest thing to deliver |
| `knowledge.SuggestionAccepted` | Knowledge | `{ suggestionId, sessionId, forDate, sport, draft, sourceLinkIds }` — **widened in P7, and the original could not work**: this row typed the payload `{ suggestionId, sessionId? }` while its own consumer column said Training fills a session, and there is no session content in a pointer. Following it would mean Training reading `suggestions`, which is the boundary violation the event exists to prevent — the same defect P2 shipped when `TaskScheduled` omitted `title` and every notification said "Task due" | Training → **fills** the named session. Not *creates*: a suggestion is generated from a session and always has one, so the creating half was unreachable, and building it would have meant inventing an hour of the day the member never chose |
| `knowledge.SuggestionDismissed` | Knowledge | `{ suggestionId, sessionId }` | (informational; the refusal is a row, so the same session is not proposed again) |
...
| `SessionMaterialiserSaga` (Training) | `SportsChanged`, `SlotsChanged`, `ProgramApplied`, `profile.PreferencesChanged` naming the zone or the cut-off, and a **nightly** internal pass | materialises `planned` sessions over `training.materialiseDays`, upserting a **derived** `_id` (`uuidv5(userId:slotId:localDate)`) so a redelivered event and two concurrent passes collapse onto one row without a lookup and without a second unique index; fills each new session from the active program's week for that date; recomputes `plannedAt` for future sessions when the member's zone changed; removes only *future* `planned` sessions whose slot is gone, and never a session with no `slotId`. The blueprint said this ran on "the existing nightly tick" — there was none, so P6 adds `POST /internal/training/materialise` and one nightly workflow, stamping `ops_heartbeats['training.materialise']` |
