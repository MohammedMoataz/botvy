...
- [ ] T420 `features/send-message/` over the socket: `chat.send` → ack → `chat.accepted`, `chat.intent`, optional `chat.moved`, `chat.token`×n with a 15 s heartbeat, `chat.card` for list intents (tasks, reminders, meetings, plan), `chat.done` with usage and actions, `chat.error` with a code
...
- [ ] T451 [P] `test/intent-fixture.mjs` + `test/fixtures/intent-cases.json` — 40 sentences (EN + AR, including stated facts like "I weigh 80 kg" and list requests) run against the live model, reporting the hit rate for SC-002
- [ ] T452 [P] Arabic strings and RTL for the chat screens
- [ ] T453 [P] `purge-on-deleted` handler for `conversations`, `messages`, `counters` and the member's own `quick_questions` on `identity.UserDeleted`; close their sockets; spec
- [ ] T454 Record gate evidence (including the fixture hit rate); open `019-meetings-calendar`
