...
- [x] T1120 `docs/parity.md` — every old capability named against its new equivalent, checked off with evidence, including that a member banned in the old system is still banned in the new one: name each of them, confirm the new system refuses their sign-in, and record it. **The file was deleted with v1 on 13 September**: its whole subject was a system that no longer exists. The finding it produced — the rate-limit regression — is fixed and lives in T1110
- [x] T1121 Move remaining members: tell them what changed, point them at the new build, confirm each signs in — and that each member the old system had banned still cannot. **Nobody to move.** The archived list had 35 accounts and **zero banned**; four were real people (`admin`, `alice@example.com`, `bob@example.com`, the Owner) and the other thirty-one were end-to-end test detritus. All four already use v2
- [x] T1122 Stop the old stack; archive its database dump and data volumes outside the repository; record where. Stopped and archived 2026-09-11 to `../botvy-v1-archive/2026-09-11` (21 MB: `v1-all.sql`, `v1-members.csv`, and a tar per volume). **The archive was then deleted on 2026-09-13 at the Owner's request**, along with the older dumps under `backups/`. There is no copy of v1's data any more, deliberately
- [x] T1123 Remove `legacy/` in one commit; confirm the build, tests and images are green without it. 234 tracked files, plus 4.6 GB of untracked Flutter build output that came back as free disk. Every ignore entry, exclude and comment naming it went in the same commit — a dead rule reads as a live one to whoever finds it next. `infra/reset.mjs` went too: archiving and retiring v1 was its whole job
- [x] T1124 Migration dropping the v1 tables from the identity database, only after T1122; the `LegacyProfileReader` from P1 removed with it. **Moot rather than done, and that is worth writing down.** The reset rebuilt PostgreSQL from nothing on 11 September, so the tables this would drop have never existed in this database; the Prisma schema has only ever declared what Identity owns. There was no `LegacyProfileReader` in the tree either — P1 described one and never built it. A forward-only migration dropping tables that are not there would be a migration whose only effect is to exist

## Phase 4 — Optional import (US4)

- [~] T1130 [P] **Struck — the archive it reads no longer exists** (T1122). `infra/import-v1.mjs` — read the archived v1 reminders and conversations and create them through the API, calling a scoped internal endpoint as a machine principal rather than the member commands, which do not accept a caller-supplied time and must not start; that endpoint is the only path on which `originalAt` and `importedFrom` are honoured. An imported message is appended and takes the next `seq` like any other, so nothing already on a phone is edited and the pull cursor still works; its place in the history comes from the original time it carries. Times map through each member's recorded zone; idempotent on the v1 identifier
- [~] T1131 [P] **Struck with T1130.** Run it if the Owner wants it; verify a member's history and that a second run changes nothing; delete the script afterwards
