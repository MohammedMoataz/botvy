...
/**
 * FR-012: the day's meetings, in the evening proposal for tomorrow **and** in
 * the morning briefing for today.
 *
 * The second half is the one that gets forgotten — the evening proposal is what
 * anybody testing this by hand would look at — so both are asserted here, and
 * so is the case that has to stay unchanged: a member with no meetings must
 * read the sentence they read before this phase, with no orphan "Meetings:"
...
function bench(): Bench {
  ...
  const tick = new TickHandler(
    ...
    tasks,
    transcript,
    new DraftBuilder(tasks, meetings, new NoSessions(), new NoMeals(), settings),
    settings,
    heartbeats,
  );
