...
class NoClients extends ServiceClientRepository {
  ...
  async upsert(_c: ServiceClientUpsert): Promise<ServiceClient> {
    throw new Error('unused');
  }
  async verifyToken(): Promise<ServiceClient | null> {
    this.calls += 1;
    return null;
  }
...
describe('guards on a service route', () => {
  it('JwtAuthGuard steps aside on a @ServiceOnly route with no bearer token', () => {
    ...
    const guard = new JwtAuthGuard(
      reflector,
      new JwtVerifier({ JWT_ACCESS_SECRET: 'x'.repeat(16) }),
    );
  ...
  it('ServiceTokenGuard steps aside on a member route and never looks a token up', async () => {
    ...
    expect(
      await new ServiceTokenGuard(clients, reflector).canActivate(context),
    ).toBe(true);
  ...
  it('ServiceTokenGuard still refuses a service route with no token', async () => {
    ...
    await expect(
      new ServiceTokenGuard(new NoClients(), reflector).canActivate(context),
